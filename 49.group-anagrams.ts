...
function groupAnagrams(str: string[]): string[][] {
  ...
  for (let i = 0; i < str.length; i++) {
    if (fetchedSet.has(str[i])) {
      continue;
    }
    const tempArr: string[] = [str[i]];
    fetchedSet.add(str[i]);
    for (let j = i + 1; j < str.length; j++) {
      if (isAnagram(str[i], str[j])) {
        fetchedSet.add(str[j]);
        tempArr.push(str[j]);
      }
    }
    ans.push(tempArr);
...
const isAnagram = (str1: string, str2: string) => {
  const frqObj1: Record<string, number> = {},
    frqObj2: Record<string, number> = {};
  for (let i = 0, j = 0; i < str1.length || j < str2.length; i++, j++) {
    if (i < str1.length) {
      frqObj1[str1[i]] = frqObj1[str1[i]] ? ++frqObj1[str1[i]] : 1;
    }
    if (j < str2.length) {
      frqObj2[str2[j]] = frqObj2[str2[j]] ? ++frqObj2[str2[j]] : 1;
    }
  }
